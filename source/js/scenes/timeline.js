	...
	// sub-scene: timeline-world
		.then(function(){
			var $subscene = $('<div class="timeline timeline-world"/>')
						.on('transitionend webkitTransitionEnd', function(e){
							if( e.currentTarget == e.target
								...
								&& parseFloat($subscene.css('opacity')) === 0
							){
								$subscene.remove()
							}
						})
						...
						.appendTo($scene)
				,$slide = $('<div class="slide"/>').appendTo($subscene)
				,$line = $('<div class="line"/>').on('transitionend webkitTransitionEnd', function(e){
							if( e.currentTarget == e.target
								&& e.originalEvent.propertyName == 'margin-left'
							){
								setTimeout(function(){
									$subscene.css('opacity', 0)
									deferred.resolve()
								}, 1500)
							}
						}).appendTo($subscene)
			...
			// slide
				for(var i=1; i<7; i++){
					/*
					$('<span class="image image-' + i + '"/>')
						.append($('<span/>').css('background-image', 'url(assets/images/scene-timeline-' + i + '.jpg)'))
						.appendTo($slide)
						*/
					$('<img/>',{
						'src':	'assets/images/scene-timeline-' + i + '.jpg'
					}).appendTo($slide)
				}
